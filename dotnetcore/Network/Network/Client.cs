...
namespace InfServer.Protocol
{
    ...
    {	// Member variables
        ...
        private bool _bClientConn;				//Are we part of a server to server connection?
        ...
        {	//Sync up!
            using (DdMonitor.Lock(_sync))
            {	//Send out a disconnect packet to only our players client
                if (_playerConn)
                {
    ...
    {	// Member variables
        ...
        {	//Redirect
            if (_obj != null)
                //Note: This calls player.destroy()
                _obj.destroy();
